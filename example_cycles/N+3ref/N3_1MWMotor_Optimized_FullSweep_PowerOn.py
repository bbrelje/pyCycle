...
def run_model(pressure_drop, data_fp = None, record=False):
    ...
    thrust3d = np.load('new_thrust3d_500kW.npy')
    T43d = np.load('new_T43d_500kW.npy')
    Wf3d = np.load('new_Wf3d_500kW.npy')
    mach3d = np.load('new_mach3d_500kW.npy')
    alt3d = np.load('new_alt3d_500kW.npy')
    throt3d = np.load('new_throt3d_500kW.npy')
    SMNmat = np.load('new_SMNmat_500kW.npy')
    SMWmat = np.load('new_SMWmat_500kW.npy')
    ...
    for i, alt in enumerate([35000., 30000., 25000., 20000., 15000., 10000., 5000., 0.]):
        for j, MN in enumerate([0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]):
            ...
            for k, throttlefrac in enumerate(throttlelist):
                ...
                np.save('new_thrust3d_500kW', thrust3d)
                np.save('new_T43d_500kW', T43d)
                np.save('new_Wf3d_500kW', Wf3d)
                np.save('new_mach3d_500kW', mach3d)
                np.save('new_alt3d_500kW', alt3d)
                np.save('new_throt3d_500kW', throt3d)
                np.save('new_SMNmat_500kW', SMNmat)
                np.save('new_SMWmat_500kW', SMWmat)
