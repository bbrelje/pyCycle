...
from N3ref_v1_convergence import N3, viewer, dump_guess, load_guess
import numpy as np 

def setup_fresh_prob(pressure_drop, data_fp = None, record=False):
    prob = om.Problem()
    ...
    if record:
        ...
        if os.path.exists(data_fp):
            ...
            file = file.replace('.','')
            prob_rec = om.SqliteRecorder(data_fp + "/maxthrottlesweep.sql")
        else:
    ...
    for powerval in [1000.]:
        prob.set_val('CRZ.motor.power', powerval, units='kW')
        prob.run_model()
...
def run_model(pressure_drop, data_fp = None, record=False):
    ...
    prob = setup_fresh_prob(pressure_drop, data_fp=data_fp, record=record)
    ...
    fn_100 = np.load('fn.npy')
    alt_fn_100 = np.load('alts.npy')
    mach_fn_100 = np.load('machs.npy')

    # thrust3d = np.zeros((8,7,11))
    # T43d = np.zeros((8,7,11))
    # Wf3d = np.zeros((8,7,11))
    # mach3d = np.zeros((8,7,11)) 
    # alt3d = np.zeros((8,7,11)) 
    # throt3d = np.zeros((8,7,11)) 
    # SMNmat = np.zeros((8,7,11)) 
    # SMWmat = np.zeros((8,7,11)) 

    thrust3d = np.load('thrust3d_1MW.npy')
    T43d = np.load('T43d_1MW.npy')
    Wf3d = np.load('Wf3d_1MW.npy')
    mach3d = np.load('mach3d_1MW.npy')
    alt3d = np.load('alt3d_1MW.npy')
    throt3d = np.load('throt3d_1MW.npy')
    SMNmat = np.load('SMNmat_1MW.npy')
    SMWmat = np.load('SMWmat_1MW.npy')
    ...
    for i, alt in enumerate([35000., 30000., 25000., 20000., 15000., 10000., 5000., 0.]):
        for j, MN in enumerate([0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]):
            fullthrustval = fn_100[i,j]

            # separate rules for zero altitude
            if alt < 4999.:
                if MN > 0.7:
                    # M > 0.7 fails to converge at SL
                    print('Skipping excessive SL mach')
                    continue
            if alt_fn_100[i,j] != alt or mach_fn_100[i,j] != MN:
                raise ValueError

            for k, throttlefrac in enumerate(throttlelist):
                if thrust3d[i,j,k] > 0.0:
                    print('======Skipping MN ' +str(MN)+' alt '+str(alt)+' throttle: '+str(throttlefrac)+'===================')
                    continue
                if throttlefrac < 0.10 or alt > 30000.:
                    continue
                print('======Running MN ' +str(MN)+' alt '+str(alt)+' throttle: '+str(throttlefrac)+'===================')
                prob['CRZ:MN'] = MN
                prob['CRZ:alt'] = alt
                prob.set_val('CRZ:Fn_target', throttlefrac*fullthrustval, units='lbf')
                try:
                    prob.run_model()
                    for pt in ['CRZ']:
                        viewer(prob, pt)
                    ...
                    thrust3d[i,j,k] = prob['CRZ.perf.Fn'][0]
                    T43d[i,j,k] = prob['CRZ.burner.Fl_O:tot:T'][0]
                    Wf3d[i,j,k] = prob['CRZ.burner.Wfuel'][0]
                    mach3d[i,j,k] = MN
                    alt3d[i,j,k] = alt
                    ...
                    if record:
                        prob.record(case_name='mach_'+str(MN)+'_alt_'+str(alt)+'_throt_'+str(int(throttlefrac*100)))
                except Exception as e:
                    print(e)
                    if str(e).split('\n')[-1] == r"Solver 'NL: Newton' on system '': residuals contain 'inf' or 'NaN' after 0 iterations.":
                        errflag = -2.0
                        prob = setup_fresh_prob(pressure_drop, data_fp=data_fp, record=record)
                    if str(e).split('\n')[-1] == r"array must not contain infs or NaNs" or str(e).split('\n')[-1] == f'divide by zero encountered in log':
                        errflag = -2.0
                        prob = setup_fresh_prob(pressure_drop, data_fp=data_fp, record=record)
                    if str(e).split('\n')[-1] == r"NL: NewtonSolver 'NL: Newton' on system '': residuals contain 'inf' or 'NaN' after 0 iterations.":
                        errflag = -2.0
                        prob = setup_fresh_prob(pressure_drop, data_fp=data_fp, record=record)
                    else:
                        errflag = -1
                    thrust3d[i,j,k] = errflag
                    T43d[i,j,k] = errflag
                    Wf3d[i,j,k] = errflag
                    mach3d[i,j,k] = MN
                    alt3d[i,j,k] = alt
                    throt3d[i,j,k] = throttlefrac
                    SMNmat[i,j,k] = errflag
                    SMWmat[i,j,k] = errflag
                np.save('thrust3d_1MW', thrust3d)
                np.save('T43d_1MW', T43d)
                np.save('Wf3d_1MW', Wf3d)
                np.save('mach3d_1MW', mach3d)
                np.save('alt3d_1MW', alt3d)
                np.save('throt3d_1MW', throt3d)
                np.save('SMNmat_1MW', SMNmat)
                np.save('SMWmat_1MW', SMWmat)
