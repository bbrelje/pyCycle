...
def run_model(pressure_drop, data_fp = None, record=False):
    ...
    thrust3d = np.load('thrust3d.npy')
    T43d = np.load('T43d.npy')
    Wf3d = np.load('Wf3d.npy')
    mach3d = np.load('mach3d.npy')
    alt3d = np.load('alt3d.npy')
    throt3d = np.load('throt3d.npy')
    SMNmat = np.load('SMNmat.npy')
    SMWmat = np.load('SMWmat.npy')
        ...
        for j, MN in enumerate([0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]):
            ...
            for k, throttlefrac in enumerate(throttlelist):
                ...
                try:
                    ...
                    SMNmat[i,j,k] = -1
                    SMWmat[i,j,k] = -1
                np.save('thrust3d_new', thrust3d)
                np.save('T43d_new', T43d)
                np.save('Wf3d_new', Wf3d)
                np.save('mach3d_new', mach3d)
                np.save('alt3d_new', alt3d)
                np.save('throt3d_new', throt3d)
                np.save('SMNmat_new', SMNmat)
                np.save('SMWmat_new', SMWmat)
