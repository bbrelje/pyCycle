...
"""
Utilizes the N3_v1 model to run heat sweeps
"""

def run_model(heat_out, data_fp = None, record=False):
    """
    Encapsulated method for running the N+3 engine model
    Parameters
    ----------
    heat_out: float, The heat being taken out of the turbine exhaust
    data_fp: string, the filepath to save the heat sweep data
    ...
    if record:
        ...
        if os.path.exists(data_fp):
            probRec = om.SqliteRecorder(data_fp + "/heatSweep_" + str(int(heat_out)) + ".sql")
        else:
...
def heat_transfer_sweep():
    ...
    full_fp = fp+"data_output/heat_sweeps/raw"
    ...
    heat_sweep = [0.0, 100e3, 200.0e3, 300.0e3, 400.0e3, 500.0e3,
                  600.0e3, 700.0e3, 800.0e3, 900.0e3, 1000.0e3]  # units: W
    for val in heat_sweep:
