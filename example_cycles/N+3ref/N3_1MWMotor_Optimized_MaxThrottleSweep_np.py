...
def run_model(pressure_drop, data_fp = None, record=False):
    ...
    try:
        T4 = np.load('new_t4.npy')
        Fn = np.load('new_fn.npy')
        Wf = np.load('new_wf.npy')
        altarray = np.load('new_alts.npy')
        macharray = np.load('new_machs.npy')
        ...
        Fn = np.zeros((8, 7))
        Wf = np.zeros((8, 7))
        macharray = np.zeros((8, 7))
        altarray = np.zeros((8, 7))
    ...
    for i, alt in enumerate([35000., 30000., 25000., 20000., 15000., 10000., 5000., 0.]):
        for j, MN in enumerate([0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2]):
            ...
            if Fn[i,j] > 0.0:
                print('======Skipping MN ' +str(MN)+' alt '+str(alt)+'===================')
                continue
            if alt < 4999.:
                # can run hotter here because time limited TO thrust
                prob.set_val('eng_op_limits.T4max', 3150, units='degR')
                prob.set_val('CRZ.motor.power', 1000, units='kW')
                if MN > 0.7:
            ...
            for pt in pts:
                viewer(prob, pt)
            Fn[i,j] = prob['CRZ.perf.Fn'].copy()
            T4[i,j] = prob['CRZ.burner.Fl_O:tot:T'].copy()
            Wf[i,j] = prob['CRZ.burner.Wfuel'].copy()
            macharray[i,j] = MN
            altarray[i,j] = alt
            if j == 0:
            ...
            np.save('new_fn', Fn)
            np.save('new_wf', Wf)
            np.save('new_alts', altarray)
            np.save('new_machs', macharray)
