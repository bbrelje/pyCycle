...
def run_model(pressure_drop, data_fp = None, record=False):
    ...
    des_vars.add_output('OPR_simple', 53.6332)
    ...
    des_vars.add_output('TOC:BPR', 23.9451, units=None)
    ...
    for pt in ['TOC'] + pts:
        viewer(prob, pt)
        print('Duct 5 and 17 qdots in KW')
        print(prob.get_val(pt+'.duct5.Q_dot', units='kW'))
        print(prob.get_val(pt+'.duct17.Q_dot', units='kW'))
        print('Temps')
        print(prob.get_val(pt+'.lpt.Fl_O:stat:T', units='K'))
        print(prob.get_val(pt+'.splitter.Fl_O1:stat:T', units='K'))
        print('Pwr net')
    ...
    prob.model.list_outputs(includes=['bal*'])
    if not record:
        exit()
...
def pressure_sweep():
    ...
    fp = "../../../"
    if not os.path.exists(fp+"data_output_opt"):
        os.makedirs(fp+"data_output_opt/pressure_sweeps/raw")
    elif not os.path.exists(fp+"data_output_opt/pressure_sweeps"):
        os.makedirs(fp+"data_output_opt/pressure_sweeps/raw")
    elif not os.path.exists(fp+"data_output_opt/pressure_sweeps/raw"):
        os.makedirs(fp+"data_output_opt/pressure_sweeps/raw")

    full_fp = fp+"data_output_opt/pressure_sweeps/raw"
