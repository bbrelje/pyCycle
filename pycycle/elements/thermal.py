...
class SimpleTMS(Group):
    ...
    def initialize(self):
        ...
        self.options.declare('motor_cost_inc', default=100. / 745., desc='Motor cost per watt $/W')
        self.options.declare('motor_cost_base', default=1., desc='Motor cost base $')
        self.options.declare('motor_specific_heat', default=921.,
                             desc='Specific heat of motor in J/kg/K - default 921 for Al')
    ...
    def setup(self):
        ...
        self.add_subsystem('motor_cold_plate',
                           ...
                           promotes_inputs=[('channel_width', 'channel_width_motor'),
                                            ('channel_height', 'channel_height_motor'),
                                            ('channel_length', 'channel_length_motor'),
                                            ('n_parallel', 'n_parallel_motor')])
        ...
        self.add_subsystem('refrigerator_cold_plate',
                           ConstantSurfaceTemperatureColdPlate_NTU(num_nodes=nn,
                                                                   ...
                                                                   nusselt=self.options['coolant_nusselt'],
                                                                   specific_heat=self.options['coolant_specific_heat']),
                           promotes_inputs=[('channel_width', 'channel_width_refrig'),
                                            ('channel_height', 'channel_height_refrig'),
                                            ('channel_length', 'channel_length_refrig'),
                                            ('n_parallel', 'n_parallel_refrig')])

        self.add_subsystem('coolant_flow', ExecComp('mdot_out = mdot_in', mdot_out={'units': 'kg/s', 'shape': (nn,)},
                                                    mdot_in={'units': 'kg/s', 'shape': (nn,)}),
                           promotes_inputs=[('mdot_in', 'mdot_coolant')])
